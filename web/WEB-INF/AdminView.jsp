...
<div class="container">
    ...
    <c:forEach var="post" items="${listPosts.rows}">
        <div class="jumbotron">
            ...
            <p align="right">
                <a href="AdminController?removePost=${post.post_id}">Remove Post</a> |
                <a href="../comment.jsp?post=${post.post_id}">Post a comment</a>
            </p>
            <hr>
            <c:forEach var="comment" items="${listComments.rows}">
                <c:if test="${comment.post_id == post.post_id}">
                    <p>${comment.first_name} commented, "${comment.comment_content}" - ${comment.comment_timestamp} ---
                        <a href="AdminController?removeComment=${comment.comment_id}">Remove Comment</a></p>
                </c:if>
            </c:forEach>
        </div>
    </c:forEach>
